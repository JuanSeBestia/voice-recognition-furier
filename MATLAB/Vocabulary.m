classdef Vocabulary < handle
    properties
        %orden = audioread('audio\orden.wav');
        words = {};
    end
    ...
    methods
        ...
        %function add_orden(self, word, orden)
         %   new_word = Word(word);
            % Concatenate and train
          %  new_word.train(extract_features(cell2mat(orden)'));
           % self.words.(char(word)) = new_word;
        %end
        ...
        function prediction = recognize(self, audio_signal)
            ...
            %display (self.name);
            %display(sprintf('Recognized as %s!', char(prediction)))
        end
